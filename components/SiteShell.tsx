...
export default function SiteShell({ data, children }: { data: Bootstrap, children: React.ReactNode }) {
  ...
  const style: any = {
    '--navy': cssColor(data.theme.color_navy,'#163B63'), '--frost': cssColor(data.theme.color_frost,'#DDEAF5'),
    '--pink': cssColor(data.theme.color_pink,'#D7BEC5'), '--green': cssColor(data.theme.color_clover,'#52B848'),
    '--ink': cssColor(data.theme.color_ink,'#14212B'), '--paper': cssColor(data.theme.color_paper,'#F7F9FB'),
    '--radius': cssLength(data.theme.radius_card,'28px'), '--font-body': `'${fontFamily(bodyFont?.family,'Inter')}', Inter, sans-serif`,
    '--font-heading': `'${fontFamily(headingFont?.family,'Manrope')}', Manrope, sans-serif`
  }
  return <div className="site-theme" style={style}>
